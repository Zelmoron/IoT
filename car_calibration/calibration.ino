...
void handle_command(char c) {
  switch (current_mode) {
    ...
    case DIR_CAL:
      ...
      else if (c == 'X') {                        // × — следующая комбинация
        stop_car();
        combo_idx = (combo_idx + 1) % 4;
        move_raw(DIR_COMBOS[combo_idx][0], speed_left,
                 DIR_COMBOS[combo_idx][1], speed_right);
      }
      else if (c == 'S') {                        // □ — подтвердить
    ...
    case SPEED_BAL:
      ...
      else if (c == 'X') { edit_left = !edit_left; }  // × — переключить колесо
      else if (c == 'R') {
      ...
      else if (c == 'T') { stop_car(); current_mode = TURN_TIME; turn_idx = 0; }
      break;
    ...
    case TURN_TIME: {
      ...
      else if (c == 'S') { if (turn_idx < 3) turn_idx++; }          // □ — следующий угол
      else if (c == 'X') { turn_ms[turn_idx] = DEF[turn_idx]; }     // × — сброс
      else if (c == 'C') { if (turn_idx > 0) turn_idx--; }          // ○ — предыдущий угол
      else if (c == 'T') { stop_car(); eeprom_save(); current_mode = RUN; }
      break;
    }
